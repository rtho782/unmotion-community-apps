...
- CA installation manifest: pinned to the verified feed commit in `unmotion.xml`, with the stable filename `unmotion.plg`. Its embedded update URL follows `plugin-stable`. Advance the template pin after publishing and verifying each new stable release.
- Support: [GitHub issues](https://github.com/rtho782/unmotion/issues)

The repository is intentionally separate from the plugin source so Community Applications can consume its XML without encountering unrelated XML files.
